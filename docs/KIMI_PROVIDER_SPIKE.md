...
The spike preserves the complete assistant history as raw JSON, including `reasoning_content` and `tool_calls`, forwards `tools`/`tool_choice`, applies K3's fixed sampling policy (`temperature=1.0`, `top_p=0.95`), forwards `reasoning_effort` (`low|high|max`), supports completion-token budgets, and retains raw SSE data frames.
